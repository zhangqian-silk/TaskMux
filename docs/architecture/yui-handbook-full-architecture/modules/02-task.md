...
中央 3→4 迁移采用 `cancelled`，保留原 `retiredAt`／`retiredBy`／`retirementSummary` 与隔离证据；当前版本 5 再经 4→5 将嵌入的结束／验收历史移入只读历史导入事件，并移除 Brief 的专用版本字段。普通取消不建立隔离证明；原显式退休的隔离事实单独保留。重新开启保留此前结束说明，旧派发快照作为事件历史可读，不再作为调度输入。
