...
同步 T04／T05 后，保留远端迁移 1–6，中央 6→7 迁移采用 `cancelled`，保留原 `retiredAt`／`retiredBy`／`retirementSummary` 与隔离证据；当前版本 8 再经 7→8 将嵌入的结束／验收历史移入只读历史导入事件，并移除 Brief 的专用版本字段。普通取消不建立隔离证明；原显式退休的隔离事实单独保留。重新开启保留此前结束说明，旧派发快照作为事件历史可读，不再作为调度输入。

## 3. Brief 编辑

Brief 内容保持通用自由文本。更新接口在数据库事务内读取最新记录，只修改指定字段，不要求版本令牌；不同字段互不覆盖，同一字段以后一次明确写入为准。

currentFocus 可以用于进度和下一步关注点。修改它与修改方案都不会自动撤销 Candidate 或 WorkItem acceptance。重要选择用 Decision 表达，Core 不做语义分类。

修改来源和前后值与写入在同一事务保存为事件，便于查询规划怎样形成或找回被覆盖的旧值。历史用于理解，由 Leader 决定是否重新写入，不自动回滚或参与另一个调度状态机。旧版本未记录的编辑内容不能凭空恢复。
