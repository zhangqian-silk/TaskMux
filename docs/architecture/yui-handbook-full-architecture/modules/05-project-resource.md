...
需要创建资源时先 prepare，返回临时引用和准备结果，再由调用方采用到 Task／AgentRun。采用前重新确认相关目标、授权及资源配置仍适用。没有采用的临时资源可以清理。
...
Role 配置改变和 AgentRun terminal 不是所有资源已停止写入的证明。只有进行冲突操作时才需要相关停止或隔离证据。可以直接使用独立资源绕开等待，不必全 Task 停住。

清理按资源所有权和引用执行。用户拥有的目录不因为 Task 结束被自动删除；共享原生服务不被当作某个 AgentRun 的子进程杀掉。具体插件知道哪些句柄和文件是它实际创建的。
