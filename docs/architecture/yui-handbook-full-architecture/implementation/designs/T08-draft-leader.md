...
若资源可安全准备且交付目录与 planning 目录不重叠，可以立即原子采用；已有 planning Session 保持原工作区和捕获的 planning 权限，不热升权，也不自唤醒。若采用确实依赖 planning 执行结束，T08 可在现有操作记录中保存 afterPlanningRun 并立即返回真实请求引用；不能让同步工具等待自己。当前 Core 在目录冲突时返回明确诊断，不伪造已排队的操作。
...
不要求所有协议具备透明 handover，也不为保留聊天而在当前 AgentRun 中热升权。逻辑 Leader 不变，用户仍在同一个 Task 对话目标下工作。
...
S01／S02：用户在 Draft 对齐并持久化，关闭会话后再次进入能继续。S27：无资源准备的任务可激活。S40：当前 AgentRun 发起延后请求即时返回，结束后采用；取消或改变实际资源条件能够被正确识别。
