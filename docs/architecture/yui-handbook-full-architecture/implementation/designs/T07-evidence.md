...
## 12. turn-17 两项已确认问题修复

本轮在 Task main 直接修复两个可复现问题，不扩大架构范围：

- `config agent update --adapter acp` 重述当前方案时保留明确的
  `claude-agent-sdk` 身份；只有真正切换方案且未显式选择组件时才推导默认。
  补丁依据事务内读到的现状生成；真实身份变更的引用保护不变。
- ACP 每次新建或恢复连接的首个 prompt 都携带短 Session Manifest 指针，
  因为已创建的 Session 可能在第一条输入前断开。后续同连接输入不重复；
  不增加持久确认状态，不重放 Task 全文。

临时测试调用实际命令与 SQLite Store，以及实际 ACP Session 类和内存
协议对端。修改前 **9 项中 4 失败、5 通过**，分别复现身份被覆盖、
引用导致无变化操作误拒、连同命令更新时身份丢失，以及恢复时缺失引导。
修改后 **9/9 通过**，并确认实际方案切换、显式身份变更及引用保护仍有效。

`make install-local`（含 build）和 lint 通过；清除继承 YUI 环境后，
在临时 Home 中一次执行三个 core 文件，**81/81 通过，0 跳过，
约 4.34 秒**。原独立审查者只读复审差异及临时测试，未发现可达问题。
临时夹具交付前移除，永久 suite 不增长。

无持久 schema 变化，storage 版本保持 11；未调用真实模型、读取凭据、
