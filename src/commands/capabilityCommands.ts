...
export async function runCapabilityCommand(
  ...
  if (!["search", "describe", "call"].includes(action)) throw usageError(usage);
  ...
  if (!taskId || positionals.length > 1 || (action !== "search" && !positionals.length)) throw usageError(usage);
  if (action === "search" && [...flags.keys()].some((key) => key !== "--task")) throw usageError(usage);
